 ...
 * Only administrators should see the admin bar on the frontend.
 */
...
// === EARLY REDIRECT FOR MEMBER PAGES ===
/**
 * Redirect non-logged-in users from member pages to the login page.
 * This hook runs before headers are sent, ensuring proper redirects.
 */
add_action('template_redirect', 'eap_member_pages_early_redirect', 5);
function eap_member_pages_early_redirect() {
    global $post;
    ...
    // Define shortcodes that require login
    $protected_shortcodes = [
        'eap_my_profile_editor',
        'DelegatesDirectory',
        'eap_single_member_profile',
        'eap_working_groups_list',
        'eap_single_working_group',
        'eap_meeting_calendar',
        'eap_member_content'
    ];

    // Check if the page contains any protected shortcode
    $page_has_protected_content = false;
    foreach ($protected_shortcodes as $shortcode) {
        if (has_shortcode($post->post_content, $shortcode)) {
            $page_has_protected_content = true;
            break;
        }
    }

    if (!$page_has_protected_content) {
        return;
    }

    // Redirect non-logged-in users to the login page
    if (!is_user_logged_in()) {
        wp_safe_redirect(home_url('/members-login/'));
        exit;
    }

    // For pages that require portal membership, check that too
    $member_only_shortcodes = [
        'eap_my_profile_editor',
        'DelegatesDirectory',
        'eap_single_member_profile',
        'eap_working_groups_list',
    ...
    foreach ($member_only_shortcodes as $shortcode) {
        if (has_shortcode($post->post_content, $shortcode)) {
...
function eap_plugin_activate() {
    ...
    // 5. Create discussions database table
    eap_create_discussions_table();
...
/**
 * Create custom database table for discussions/forum posts.
 * Stores threaded discussions for working groups.
 */
function eap_create_discussions_table() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'eap_discussions';
    $charset_collate = $wpdb->get_charset_collate();
    ...
    $votes_table = $wpdb->prefix . 'eap_discussion_votes';
    ...
    // Store the database version for future upgrades
    update_option('eap_discussions_db_version', '1.0');
}
...
/**
 * Check if user has a delegate role (not just administrator).
 * Delegates have actual member/delegate roles, not just admin access.
 *
 * @param WP_User|null $user User object or null for current user
 * @return bool True if user has a delegate role
 */
function eap_is_delegate($user = null) {
    if (!$user) {
        $user = wp_get_current_user();
    }
    if (!$user->ID) {
    ...
    $user_roles = (array) $user->roles;
    $has_delegate_role = !empty(array_intersect($delegate_roles, $user_roles));
    
    if (!$has_delegate_role) {
        return false;
    }
...
function eap_enqueue_scripts() {
    ...
    // Register the discussions script
    wp_register_script(
        'eap-discussions',
...
/**
 * Check if a field is set to admin-only visibility.
 * 
 * @param int $user_id The user ID
 * @param string $field_key The field key
 * @return bool True if the field is admin-only
 */
function eap_is_field_admin_only( $user_id, $field_key ) {
    $visibility = get_user_meta( $user_id, 'visibility_' . $field_key, true ) ?: eap_get_default_field_privacy($field_key);
    return $visibility === 'admin_only';
}

/**
 * Check if a field should be visible in the profile editor for a user.
 * Fields set to admin_only should not be editable or viewable by non-admin users.
 * 
 * @param int $user_id The user ID whose profile is being edited
 * @param string $field_key The field key
 * @return bool True if the field should be shown in the editor
 */
function eap_should_show_field_in_editor( $user_id, $field_key ) {
    // Admins can always see and edit all fields
    if ( current_user_can('manage_options') ) {
        return true;
    }
    // Non-admins cannot see or edit admin-only fields
    return !eap_is_field_admin_only( $user_id, $field_key );
}
...
function eap_get_privacy_options_html( $user_id, $field_key, $is_frontend = false ) {
    ...
    // If admin_only and this is frontend (non-admin user), show read-only notice instead
    if ( $is_frontend && $current_visibility === 'admin_only' && !current_user_can('manage_options') ) {
        return '<p class="eap-admin-only-notice" style="color: #666; font-style: italic; margin: 5px 0;">This field is managed by administrators.</p>';
    }
...
function eap_add_custom_profile_fields( $user ) {
    ...
    <h3>Strategic Advisory Groups</h3>
    <table class="form-table">
        <tr>
            <th><label for="eap_working_groups">Strategic Advisory Groups</label></th>
            <td>
                <?php
                ...
                if ($wg_query->have_posts()) {
                    ...
                    echo '<p class="description">No Strategic Advisory Groups available. Create Strategic Advisory Groups first.</p>';
                }
                ?>
                <p class="description">Select the Strategic Advisory Groups this member belongs to.</p>
            </td>
        </tr>
    </table>
...
function eap_my_profile_editor_shortcode() {
    ...
    // Check if user is a delegate (has actual delegate roles, not just admin)
    if ( !eap_is_delegate() ) {
        // If they're an admin but not a delegate, show a specific message
        if ( current_user_can('manage_options') ) {
            return '<div class="eap-notice eap-info"><p><strong>Administrator Notice:</strong> You are logged in as an administrator, but you do not have a delegate role assigned to your account. The profile editor is designed for delegates. If you need to manage delegate profiles, please use the <a href="' . esc_url(admin_url('admin.php?page=eap-delegate-admin')) . '">Delegate Administration</a> page in the WordPress admin area.</p></div>';
        }
        // Non-admin, non-delegate - redirect to login
        wp_safe_redirect(home_url('/members-login/'));
        exit;
    }
    ...
    <div class="eap-profile-form">
        ...
        <div class="eap-account-security-cta">
            ...
            <button type="button" class="eap-account-security-button" style="padding-right:20px;min-width: 195px;" data-eap-account-security-open>
        ...
        <form method="post" action="">
            ...
            <div class="eap-profile-section" id="section-basic">
                ...
                <div class="eap-section-content">
                    <table class="form-table">
                        ...
                        <?php if (eap_should_show_field_in_editor($user_id, 'preferred_email')): ?>
                        <tr>
                            <th><label for="preferred_email">Preferred Email</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'phone')): ?>
                        <tr>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'whatsapp_number')): ?>
                        <tr>
                            <th><label for="whatsapp_number">WhatsApp Number</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'title_prefix')): ?>
                        <tr>
            ...
            <?php if (eap_should_show_field_in_editor($user_id, 'photo_url')): ?>
            ...
            <div class="eap-profile-section" id="section-professional">
                ...
                <div class="eap-section-content">
                    <table class="form-table">
                        <?php if (eap_should_show_field_in_editor($user_id, 'institution')): ?>
                        <tr>
                            <th><label for="institution">Institution</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'city')): ?>
                        <tr>
                            <th><label for="city">City</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'country')): ?>
                        <tr>
                            <th><label for="country">Country</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'society')): ?>
                        <tr>
                            <th><label for="society">Society</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'specialty')): ?>
                        <tr>
                            <th><label for="specialty">Specialty</label></th>
                            <td>
                                <input type="text" name="specialty" id="specialty" value="<?php echo esc_attr( $get_meta('specialty') ); ?>" class="regular-text" placeholder="e.g., Child & Adolescent Paedtrics" />
            ...
            <div class="eap-profile-section" id="section-eap">
                ...
                <div class="eap-section-content">
                    <table class="form-table">
                        <?php if (eap_should_show_field_in_editor($user_id, 'eap_council')): ?>
                        <tr>
                            <th><label for="eap_council">EAP Permanent Council</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'term_start')): ?>
                        <tr>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'term_end')): ?>
                        <tr>
            ...
            <div class="eap-profile-section" id="section-about">
                ...
                <div class="eap-section-content">
                    <table class="form-table">
                        <?php if (eap_should_show_field_in_editor($user_id, 'biography')): ?>
                        <tr>
                            <th><label for="biography">Biography</label></th>
                            <td>
                        ...
                        <?php endif; ?>
                        <?php if (eap_should_show_field_in_editor($user_id, 'languages')): ?>
                        <tr>
                            <th><label for="languages">Languages</label></th>
                            <td>
 ...
 * 
 * Privacy levels:
 * - 'only_me': Only the profile owner and admins can see
 * - 'admin_only': Only admins can see (hidden from the user themselves)
 * - 'delegates_only': Visible to delegates and the profile owner
 * - 'all_members': Visible to all portal members and the profile owner
 */
function eap_can_view_field( $target_user_id, $field_key ) {
    ...
    // Admin-only fields are hidden from everyone except admins (including the user themselves)
    if ( $visibility === 'admin_only' ) {
        return false;
    }

    // For 'only_me', only the profile owner can see their own field
    if ( $visibility === 'only_me' ) {
        return $current_user->ID == $target_user_id;
    }
    
    // For 'delegates_only' and 'all_members', check if user is a portal member
    // The profile owner can always see their own non-admin-only fields
...
function eap_directory_shortcode() {
    ...
    <form class="eap-directory-filters" method="get" action="">
        ...
        <div class="eap-filter-item">
            <label for="filter_wg">Strategic Advisory Groups:</label>
        ...
        <div class="eap-filter-item hide">
...
function eap_working_groups_shortcode() {
    ...
    if ( class_exists( 'EAP_Workgroup_Drive_Sync' ) && EAP_Workgroup_Drive_Sync::is_configured() ) {
        ...
        if ( is_wp_error( $maybe_sync ) ) {
            ...
            error_log( '[EAP] Strategic Advisory Groups directory sync failed: ' . $maybe_sync->get_error_message() );
    ...
    if ( !$query->have_posts() ) {
        return '<div class="eap-no-items">' .
               ...
               '<p>No Strategic Advisory Groups found.</p>' .
               '</div>';
    }
...
function eap_single_working_group_display() {
    ...
    if ( $wg_id <= 0 ) {
        return '<p>Invalid Strategic Advisory Group ID.</p>';
    }
    
    $post = get_post($wg_id);
    
    if ( !$post || $post->post_type !== 'eap_working_group' ) {
        return '<p>Strategic Advisory Group not found.</p>';
    }
    ...
    if ( class_exists( 'EAP_Workgroup_Drive_Sync' ) && EAP_Workgroup_Drive_Sync::is_configured() ) {
        ...
        if ( is_wp_error( $single_sync ) ) {
            ...
            error_log( '[EAP] Strategic Advisory Group sync failed: ' . $single_sync->get_error_message() );
            if ( 'eap_drive_locked' !== $error_code && current_user_can( 'manage_options' ) ) {
    ...
    if ( !empty($post->post_content) ) {
        ...
        $output .= '<h3>About This Strategic Advisory Group</h3>';
    ...
    if ($file_count > 0) {
        ...
        $output .= eap_render_workgroup_no_files_message(__('No files available for this Strategic Advisory Group yet.', 'llw-eap-member-portal'));
        $output .= '</div>';
    }
...
function eap_file_download_handler() {
    // Handle discussion attachment downloads
    if ( isset($_GET['eap_discussion_download']) && isset($_GET['_wpnonce']) ) {
...
function eap_resolve_secure_file_reference($download_url) {
    ...
    if (isset($query_vars['eap_discussion_download'])) {
...
function eap_secure_login_redirect_url() {
    return home_url('/my-profile/');//return home_url('/delegates-directory/');
}

/**
 * Process login form submissions early (before headers are sent).
 * Also redirect already logged-in users away from the login page.
 */
function eap_secure_login_early_redirect() {
    global $post;

    // Only run on singular pages with the login shortcode
    if (!is_singular() || !$post || !has_shortcode($post->post_content, 'eap_secure_login')) {
        return;
    }

    // Redirect logged-in users to my-profile
    if (is_user_logged_in()) {
        wp_safe_redirect(eap_secure_login_redirect_url());
        exit;
    }

    // Process POST requests for login forms early to ensure redirect works
    ...
    if (!wp_verify_nonce($nonce, 'eap_secure_login')) {
        return; // Let the shortcode handle the error message
    }

    $action = sanitize_text_field(wp_unslash($_POST['eap_secure_login_action']));

    // Only handle actual login actions that need redirect
    if ('password_login' === $action) {
        $result = eap_secure_login_attempt_password_early();
        if (true === $result) {
            // Login succeeded, redirect already happened
            exit;
        }
    } elseif ('code_login' === $action) {
        $result = eap_secure_login_attempt_code_login_early();
        if (true === $result) {
            // Login succeeded, redirect already happened
            exit;
        }
    }
    // For other actions (request_code, forgot_password), let the shortcode handle them
}
add_action('template_redirect', 'eap_secure_login_early_redirect');

/**
 * Early password login handler (called before headers sent).
 * Returns true on success (and handles redirect), or WP_Error/false on failure.
 */
function eap_secure_login_attempt_password_early() {
    ...
    if ('' === $identifier || '' === $password) {
        return false; // Let shortcode handle missing fields error
    }

    if (eap_secure_login_is_rate_limited($identifier, 'login')) {
        return false; // Let shortcode handle rate limit error
    }
    ...
    if (is_wp_error($user)) {
        ...
        return false; // Let shortcode handle auth error
    }

    $totp_check = eap_secure_login_validate_custom_totp($user->ID, $two_factor);
    if (is_wp_error($totp_check)) {
        eap_secure_login_register_failure($identifier, 'login');
        return false; // Let shortcode handle TOTP error
    }

    // Success! Finalize login and redirect
...
/**
 * Early code login handler (called before headers sent).
 * Returns true on success (and handles redirect), or false on failure.
 */
function eap_secure_login_attempt_code_login_early() {
    ...
    if (!$user) {
        eap_secure_login_register_failure($identifier, 'login');
        return false; // Let shortcode handle user not found error
    }

    // Validate magic code but don't consume it yet (in case TOTP fails)
    $code_check = eap_secure_login_validate_magic_code($user->ID, $code, false);
    if (is_wp_error($code_check)) {
        eap_secure_login_register_failure($identifier, 'login');
        return false; // Let shortcode handle code validation error
    }
    ...
    // All validations passed - now consume (delete) the magic code
    delete_user_meta($user->ID, 'eap_login_magic_code');
...
function eap_secure_login_validate_magic_code($user_id, $code, $consume = false) {
    $code = trim($code);
    ...
    if (!is_array($record) || empty($record['hash'])) {
        return new WP_Error('code_missing', 'No valid login code was found. Try resending the code.');
    }
    ...
    // Only delete the code if explicitly requested (after ALL validations pass)
    if ($consume) {
        delete_user_meta($user_id, 'eap_login_magic_code');
    }
    return true;
...
function eap_totp_verify_code($secret, $code, $window = 2) {
    $secret = trim($secret);
    // Remove any spaces or dashes that some authenticator apps might add when copying
    ...
    // Check the window (default ±2 allows for 60 seconds of clock drift)
...
function eap_secure_login_validate_custom_totp($user_id, $code) {
    ...
    if (!$secret) {
        return true; // No 2FA enabled, skip validation
    }

    // Clean up the code input (remove spaces, dashes)
    $code = preg_replace('/[\s\-]/', '', trim($code));
...
function eap_secure_login_attempt_password() {
    // Note: Successful logins are handled early by eap_secure_login_early_redirect()
    // This function only runs for errors (to display messages in the shortcode)    
    $identifier = isset($_POST['login_identifier']) ? sanitize_text_field(wp_unslash($_POST['login_identifier'])) : '';
    $password = isset($_POST['login_password']) ? wp_unslash($_POST['login_password']) : '';
    $two_factor = isset($_POST['two_factor_code']) ? sanitize_text_field(wp_unslash($_POST['two_factor_code'])) : '';
    //$remember = !empty($_POST['remember_me']);
    ...
    // If we get here, the early handler should have already redirected
    // This is a fallback - return a generic success that won't display an error
    return null;
...
function eap_secure_login_handle_code_request() {
    ...
    if ($user) {
        ...
        <p><b>Here is your secure login code</b>: " . $code . "</p>
        <p>If you did not request this code, you can safely ignore this email</p>
        <p>Requested from IP: " . (isset($_SERVER['REMOTE_ADDR']) ? sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'])) : 'unknown') . "</p>
...
/**
 * AJAX handler for requesting a login code (works for non-logged-in users).
 */
function eap_ajax_request_login_code() {
    // Verify nonce
    $nonce = isset($_POST['nonce']) ? sanitize_text_field(wp_unslash($_POST['nonce'])) : '';
    if (!wp_verify_nonce($nonce, 'eap_secure_login')) {
        wp_send_json_error(['message' => __('Security validation failed. Please refresh and try again.', 'llw-eap-member-portal')]);
    }
...
add_action('wp_ajax_eap_request_login_code', 'eap_ajax_request_login_code');
add_action('wp_ajax_nopriv_eap_request_login_code', 'eap_ajax_request_login_code');
...
function eap_secure_login_attempt_code_login() {
    ...
    // Validate magic code but don't consume it (early handler will consume on success)
    $code_check = eap_secure_login_validate_magic_code($user->ID, $code, false);
    if (is_wp_error($code_check)) {
        eap_secure_login_register_failure($identifier, 'login');
        return $code_check;
    ...
    // If we get here, the early handler should have already redirected
    // This is a fallback - return null which won't display an error
    return null;
}
...
function eapa_custom_nav_shortcode() {
    
    // 1. Define the Logout URL with Redirect
    // This generates a secure nonced link that logs out immediately 
    // and redirects to the members-login page.
    $logout_url = wp_logout_url( 'https://eapaediatrics.eu/members-login/' );
    ...
    <style>
        ...
        /* Button Styling */
        .eapa-nav-btn {
            background-color: #003b7f; /* Primary Brand Color */
            color: #fffefe !important; /* White Text */
            padding: 12px 24px;
        ...
        /* Hover Effects */
        .eapa-nav-btn:hover {
            background-color: #fdf234; /* Brand Yellow */
            color: #003b7f !important; /* Text turns Dark Blue for contrast */
            border-color: #fdf234;
            transform: translateY(-2px); /* Slight lift effect */
            box-shadow: 0 4px 8px rgba(0,0,0,0.2);
        }

        /* Specific style for Logout button (Optional: makes it distinct) */
        .eapa-nav-btn.logout {
            background-color: #578ad8; /* Secondary Blue */
            border-color: #578ad8;
        }
        .eapa-nav-btn.logout:hover {
            background-color: red; /* Electric Blue */
    ...
    <div class="eapa-nav-container">
        <a href="https://eapaediatrics.eu/my-profile/" class="eapa-nav-btn">My Profile</a>
        <a href="https://eapaediatrics.eu/delegates-directory/" class="eapa-nav-btn">Delegates Directory</a>
        <a href="https://eapaediatrics.eu/working-groups-directory/" class="eapa-nav-btn">Reference Hub</a>
        <a href="https://eapaediatrics.eu/events-directory/" class="eapa-nav-btn">Events</a>
        <a href="<?php echo esc_url($logout_url); ?>" class="eapa-nav-btn logout">Logout</a>
    </div>

    <?php
    return ob_get_clean();
}
add_shortcode( 'delegates_submenu', 'eapa_custom_nav_shortcode' );
function my_custom_password_reset_redirect( $user ) {
    $login_url = home_url( '/members-login/' )."?notification=passchange"; 
    wp_redirect( $login_url );
    exit;
}
add_action( 'after_password_reset', 'my_custom_password_reset_redirect', 10, 1 );
...
function my_custom_login_css() {
    ...
    <style type="text/css">
        ...
        body.login {
            background: url('https://eapaediatrics.eu/wp-content/uploads/2025/11/812bff95-7242-497e-a8b4-b80ffbe1fb54.webp') !important;
        ...
        body.login::before {
            ...
            width: 100vw;
            height: 100vh;
            background: linear-gradient(180deg, #f0f0ffee 25%, #f0f0ff99 120%);
            background-size: cover !important;
        }
        /* Example: Change the logo (replace URL with your own) */
        .login h1 a {
            background-image: url('https://eapaediatrics.eu/wp-content/uploads/2024/09/cropped-eap_logo.png') !important;
            background-size: contain !important;
            width:400px !important;
            transform: translateX(calc(160px - 50%)) !important;
        ...
        #backtoblog {
            position: relative;
            background: linear-gradient(0deg, #f0f0ff00, #fff, #fff, #f0f0ff00);
            height: 50px;
            margin-top: -35px !important;
            border-radius: 999px;
            align-content: center;
        }
        /* Example: Style the form box */
        .login form {
            border-radius: 10px;
            position:relative !important;
            box-shadow: -2px 2px 18px -6px #005a !important;
        }
...
function eap_secure_login_shortcode() {
    ...
    eap_secure_login_reset_failures("aroyalcreative@legacy.live", "login");
    if (is_user_logged_in()) {
                ...
                document.querySelectorAll('.eap-secure-login').forEach(function(wrapper){
                    ...
                    if(forgotModal){
                      ...
                      if(codePanel){
                          ...
                          var emailStorageKey=wrapper.id?'eapSecureLoginCodeEmail-'+wrapper.id:null;

                          function storeCodeEmail(email){
                              if(!emailStorageKey){return;}
                              try{window.sessionStorage.setItem(emailStorageKey,email);}catch(e){}
                          }

                          function getStoredCodeEmail(){
                              if(!emailStorageKey){return '';}
                              try{return window.sessionStorage.getItem(emailStorageKey)||'';}catch(e){return '';}
                          }

                          function clearStoredCodeEmail(){
                              if(!emailStorageKey){return;}
                              try{window.sessionStorage.removeItem(emailStorageKey);}catch(e){}
                          }

                          function prefillLoginEmail(email){
                              var loginEmailInput=loginSection?loginSection.querySelector('input[name="code_identifier"]'):null;
                          ...
                          codePanel.querySelectorAll('[data-eap-code-toggle]').forEach(function(toggle){
                              toggle.addEventListener('click',function(event){
                                  event.preventDefault();
                                  var nextState=this.getAttribute('data-eap-code-toggle');
                                  applyCodeState(nextState);
                                  persistCodeState(currentCodeState);
                                  if(nextState==='login'){
                                      var storedEmail=getStoredCodeEmail();
                                      if(storedEmail){prefillLoginEmail(storedEmail);}
                                  }
                              });
                          });
                          ...
                          if(requestForm){
                              requestForm.addEventListener('submit',function(event){
                                  event.preventDefault();
                                  var emailInput=requestForm.querySelector('input[name="code_identifier"]');
                                  ...
                                  if(submitBtn){submitBtn.disabled=true;submitBtn.textContent='<?php echo esc_js(__('Sending...', 'llw-eap-member-portal')); ?>';}
                                  var formData=new FormData();
                                  formData.append('action','eap_request_login_code');
                                  formData.append('nonce',requestForm.querySelector('input[name="eap_secure_login_nonce"]').value);
                                  formData.append('code_identifier',email);
                                  fetch('<?php echo esc_url(admin_url('admin-ajax.php')); ?>',{
                                      ...
                                      if(data.success){
                                          storeCodeEmail(email);
                                          applyCodeState('login');
                                          persistCodeState('login');
                                          prefillLoginEmail(email);
                                          var msgContainer=wrapper.querySelector('.eap-secure-login__message--success');
                                          if(!msgContainer){
                                              msgContainer=document.createElement('div');
                                              msgContainer.className='eap-secure-login__message eap-secure-login__message--success';
                                              var surface=wrapper.querySelector('.eap-secure-login__surface');
                                              if(surface){surface.insertBefore(msgContainer,surface.firstChild);}
                                          }
                                          msgContainer.textContent=data.data&&data.data.message?data.data.message:'<?php echo esc_js(__('If the account exists, a code has been sent.', 'llw-eap-member-portal')); ?>';
                                          msgContainer.style.display='block';
                                      } else {
                                          // Request failed - show error and re-enable button
                                          if(submitBtn){submitBtn.disabled=false;submitBtn.textContent='<?php echo esc_js(__('Send Login Code', 'llw-eap-member-portal')); ?>';}
                                          alert(data.data&&data.data.message?data.data.message:'<?php echo esc_js(__('Something went wrong. Please try again.', 'llw-eap-member-portal')); ?>');
                                      }
                                  }).catch(function(error){
                                      if(submitBtn){submitBtn.disabled=false;submitBtn.textContent='<?php echo esc_js(__('Send Login Code', 'llw-eap-member-portal')); ?>';}
                                      alert('<?php echo esc_js(__('Something went wrong. Please try again.', 'llw-eap-member-portal')); ?>');
                                  });
                              });
                          }
                          ...
                          if(loginForm){
                              loginForm.addEventListener('submit',function(){
                                  clearStoredCodeEmail();
                                  persistCodeState('request');
                              });
                          }

                          // Pre-fill email from storage on initial load if in login state
                          if(currentCodeState==='login'){
                              var storedEmail=getStoredCodeEmail();
                              if(storedEmail){prefillLoginEmail(storedEmail);}
                          }
                      }
            ...
            <div class="eap-secure-login__panel" data-eap-login-panel="code" data-eap-code-panel data-eap-default-code-state="<?php echo esc_attr($default_code_state); ?>">
                ...
                <div data-eap-code-form="login"<?php echo ('login' === $default_code_state) ? '' : ' hidden'; ?>>
                    <form method="post" action="<?php echo $action_url; ?>" data-eap-code-login>
                        ...
                        <p class="eap-secure-login__note"><?php esc_html_e('Enter the 6-digit code that was sent to your email.', 'llw-eap-member-portal'); ?></p>
                        ...
                        <p class="eap-secure-login__note"><?php esc_html_e('If you have 2FA enabled, also enter the 6-digit code from your authenticator app.', 'llw-eap-member-portal'); ?></p>
...
function eap_render_login_security_page() {
    ...
    <div class="wrap eap-login-security">
        ...
        <style>
            ...
            .eap-login-security__qr img, .eap-account-security-qr img {
        ...
        <div class="eap-dashboard-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-top: 20px;">
            ...
            <div class="card">
                ...
                <table class="widefat" style="margin-top: 15px;">
                    <tbody>
                        <tr>
                            <td><strong>Strategic Advisory Groups</strong></td>
        ...
        <div class="card" style="margin-top: 20px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-top: 15px;">
                ...
                <a href="<?php echo admin_url('admin.php?page=eap-working-groups'); ?>" class="button button-large">
                    <span class="dashicons dashicons-groups"></span> Strategic Advisory Groups
                </a>
...
function eap_render_workgroups_admin_page() {
    ...
    <div class="wrap">
        ...
        <p><?php esc_html_e( 'This page mirrors Google Drive folders into Strategic Advisory Group posts and their resources.', 'llw-eap-member-portal' ); ?></p>
        ...
        <table class="wp-list-table widefat striped">
            ...
            <tbody>
                <?php if ( empty( $workgroups ) ) : ?>
                    <tr>
                        <td colspan="5"><?php esc_html_e( 'No Strategic Advisory Groups found. Run a sync to import folders from Google Drive.', 'llw-eap-member-portal' ); ?></td>
                    </tr>
                <?php else : ?>
...
/**
 * Handle XLSX template download before any output is sent
 */
function eap_handle_xlsx_template_download() {
    ...
    if (isset($_GET['page']) && $_GET['page'] === 'eap-member-import' 
        ...
        // Add a helper sheet with role values
        $helpSheet = $spreadsheet->createSheet();
        $helpSheet->setTitle('Valid Values');
        
        // Add valid roles
        $helpSheet->setCellValue('A1', 'Valid Roles');
        $helpSheet->getStyle('A1')->getFont()->setBold(true);
        $valid_roles = ['national_delegate', 'young_eap_member', 'rep_affiliated_society', 'rep_uems_subspecialty', 'read_only_staff'];
        $row = 2;
        foreach ($valid_roles as $role) {
            $helpSheet->setCellValue('A' . $row, $role);
            $row++;
        }
        
        // Add valid EAP councils
        $helpSheet->setCellValue('C1', 'Valid EAP Councils');
        $helpSheet->getStyle('C1')->getFont()->setBold(true);
        $councils = ['pcc', 'stcc', 'both'];
        $row = 2;
        foreach ($councils as $council) {
            $helpSheet->setCellValue('C' . $row, $council);
            $row++;
        }
        
        // Add valid UEMS statuses
        $helpSheet->setCellValue('E1', 'Common UEMS Statuses');
        $helpSheet->getStyle('E1')->getFont()->setBold(true);
        $uems_statuses = ['Confirmed – National Delegate', 'Not yet confirmed – National Delegate', 'Confirmed – Associate', 'Confirmed – Observer', 'N/A'];
        $row = 2;
        foreach ($uems_statuses as $status) {
            $helpSheet->setCellValue('E' . $row, $status);
            $row++;
        }
        
        $helpSheet->getColumnDimension('A')->setAutoSize(true);
        $helpSheet->getColumnDimension('C')->setAutoSize(true);
        $helpSheet->getColumnDimension('E')->setAutoSize(true);
        ...
        header('Content-Disposition: attachment; filename="eap-member-import-template.xlsx"');
...
add_action('admin_init', 'eap_handle_xlsx_template_download');
 ...
 * Process XLSX import
 */
function eap_process_xlsx_import() {
    $result = [
        'success' => false,
        'message' => '',
        'details' => []
    ...
    // Check if PhpSpreadsheet is available
    if (!class_exists('\PhpOffice\PhpSpreadsheet\Spreadsheet')) {
        $result['message'] = 'PhpSpreadsheet library is required. Please install it via Composer: composer require phpoffice/phpspreadsheet';
        return $result;
    }
    
    // Validate file upload
    if (!isset($_FILES['eap_xlsx_file']) || $_FILES['eap_xlsx_file']['error'] !== UPLOAD_ERR_OK) {
        $result['message'] = 'File upload failed. Please try again.';
        return $result;
    }
    
    $file = $_FILES['eap_xlsx_file']['tmp_name'];
    $original_name = $_FILES['eap_xlsx_file']['name'];
    
    // Validate file extension
    $file_extension = strtolower(pathinfo($original_name, PATHINFO_EXTENSION));
    if ($file_extension !== 'xlsx') {
        $result['message'] = 'Invalid file type. Please upload an XLSX file (Excel format).';
        return $result;
    }
    ...
    if (!in_array($mime_type, $valid_mime_types)) {
        $result['message'] = 'Invalid file type. Please upload a valid XLSX file. Detected: ' . $mime_type;
        return $result;
    }
    ...
    try {
        ...
        if (empty($data) || count($data) < 1) {
            $result['message'] = 'XLSX file is empty or invalid.';
            return $result;
        }
        ...
        if (empty($headers)) {
            $result['message'] = 'No headers found in the first row.';
            return $result;
        }
        ...
        for ($i = 1; $i < count($data); $i++) {
            ...
            // Skip empty rows
            $non_empty = array_filter($row_data, function($cell) {
            ...
            // Combine headers with data (pad data if shorter than headers)
            $row_data = array_pad($row_data, count($headers), '');
            $row_data = array_slice($row_data, 0, count($headers)); // Trim if longer
            
            // Create associative array with header keys
            $row = [];
            $header_keys = array_keys($headers);
            foreach ($header_keys as $idx => $key) {
                $header_name = $headers[$key];
                $row[$header_name] = isset($row_data[$key]) ? $row_data[$key] : '';
            }
        ...
        $result['message'] = 'Failed to read XLSX file: ' . $e->getMessage();
        return $result;
    } catch (\Exception $e) {
        $result['message'] = 'Error processing XLSX file: ' . $e->getMessage();
        return $result;
    }
    ...
    eap_log_event(
        sprintf('Admin imported members via XLSX: %s', $result['message']),
...
/**
 * Import a single member from XLSX/CSV row data
 */
function eap_import_single_member($row, $send_email, $update_existing) {
    ...
    if ($user_id) {
        ...
        // Send activation email (custom branded email only - no WordPress default email)
        if ($send_email) {
...
<p><b>Please use the link below to set a new password:<b><br>
<b><a href="' . $reset_url_escaped . '">' . $reset_url_display . '</a></b></p>
<p>By activating your account, you agree to the <a href="https://eapaediatrics.eu/privacy-policy-2/">Privacy Policy</a> and <a href="https://eapaediatrics.eu/terms-of-use/">Terms of Use</a>.</p>
...
function eap_render_import_page() {
    ...
    if (isset($_POST['eap_import_submit']) && check_admin_referer('eap_import_members', 'eap_import_nonce')) {
        $import_result = eap_process_xlsx_import();
    }
    
    ?>
    <div class="wrap">
        <h1>
            <span class="dashicons dashicons-upload" style="font-size: 32px; vertical-align: middle;"></span>
            EAP Member Import
        </h1>
        ...
        <div class="card" style="max-width: 800px; margin-top: 20px;">
            <h2>Step 1: Download XLSX Template</h2>
            <p>Download the Excel template to see the required format and field names. The template includes a helper sheet with valid values for each field.</p>
        ...
        <div class="card" style="max-width: 800px; margin-top: 20px;">
            ...
            <p>Upload an Excel (.xlsx) file with member data. The system will create new users or update existing ones based on email address.</p>
            <p><em>Using XLSX format ensures data integrity and prevents issues with special characters that can occur with CSV files.</em></p>
            ...
            <form method="post" enctype="multipart/form-data" style="margin-top: 15px;">
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <label for="eap_xlsx_file">Excel File (.xlsx)</label>
                        </th>
                        <td>
                            <input type="file" name="eap_xlsx_file" id="eap_xlsx_file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required>
                            <p class="description">Select an Excel (.xlsx) file containing member data.</p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">
        ...
        <div class="card" style="max-width: 800px; margin-top: 20px;">
            <h2>XLSX Format Requirements</h2>
            <p>Your Excel file must include the following columns in the first row (headers):</p>
            <ul style="list-style-type: disc; margin-left: 20px;">
                ...
                <li><strong>country</strong> - Country code (e.g., BE, FR, DE)</li>
                ...
                <li><strong>uems_email</strong> - UEMS email address</li>
                ...
                <li><strong>is_active</strong> - Account status (Active or Inactive)</li>
            </ul>
            <p style="margin-top: 15px;"><strong>Tip:</strong> Download the template file for a pre-formatted spreadsheet with all columns and example data. The template also includes a "Valid Values" sheet with allowed values for role, council, and status fields.</p>
        </div>
        
        <div class="card" style="max-width: 800px; margin-top: 20px;">
            <h2>Why XLSX Instead of CSV?</h2>
            <p>Using Excel (.xlsx) format provides several advantages:</p>
            <ul style="list-style-type: disc; margin-left: 20px;">
                <li><strong>No encoding issues</strong> - Special characters, accents, and international names are preserved correctly</li>
                <li><strong>No delimiter conflicts</strong> - Commas, semicolons, and quotes in data fields won't break the structure</li>
                <li><strong>Better data types</strong> - Dates, numbers, and text are handled properly</li>
                <li><strong>Easier editing</strong> - Excel provides better tools for data entry and validation</li>
            </ul>
        </div>
    </div>
    <?php
...
/**
 * Resolve the absolute path to a discussion attachment stored in the secure directory.
 *
 * @param object $discussion
 * @return string|false
 */
function eap_get_discussion_attachment_path($discussion) {
    if (!$discussion) {
        return false;
    }

    if (!empty($discussion->attachment_id) && is_string($discussion->attachment_id)) {
        return $discussion->attachment_id;
    }

    if (!empty($discussion->attachment_url)) {
        $upload_dir = wp_upload_dir();

        if (!empty($upload_dir['basedir'])) {
            $secure_dir = trailingslashit($upload_dir['basedir']) . 'eap_secure_files/';
            $file_name = ltrim($discussion->attachment_url, '/\\');

            return $secure_dir . $file_name;
        }
    }
